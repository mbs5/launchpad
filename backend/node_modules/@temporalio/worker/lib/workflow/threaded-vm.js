...
class WorkerThreadClient {
    constructor(workerThread, logger) {
        ...
        workerThread.on('error', (err) => {
            ...
            this.exitError = new core_bridge_1.UnexpectedError(`Workflow Worker Thread exited prematurely: ${err}`, err);
        ...
        workerThread.on('exit', (exitCode) => {
            ...
            const error = this.exitError ??
                new core_bridge_1.UnexpectedError('Workflow Worker Thread exited while there were still pending completions', {
    ...
    async send(input) {
        if (this.exitError || this.workerExited) {
            throw this.exitError ?? new core_bridge_1.UnexpectedError('Received request after worker thread exited');
        }
